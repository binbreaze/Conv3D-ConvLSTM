...
def conv3d_time(input):
    ...
    conv3d2 = Conv3D(filters=20,
                       kernel_size=(3,3,3),
                       strides=(2,1,1),
                       padding='same',
                       activation='relu',
                       data_format='channels_last')
    conv3d3 = Conv3D(filters=20,
                       kernel_size=(3,3,3),
                       strides=(2,1,1),
                       padding='same',
                       activation='relu',
    ...
    conv3d1_out = conv3d1(input)
    # normal_out = normal(conv3d_out)
    conv3d2_out = conv3d2(conv3d1_out)
    # normal1_out = normal(conv3d_output)
    conv3d3_out = conv3d3(conv3d2_out)
    return conv3d3_out
...
def conv3d_convlstm(input_shape,sample_len):
    ...
    # convlstm2d层
    convlstm1 = ConvLSTM2D(filters=10,
                          kernel_size=(3,3),
                          strides=(1,1),
                          padding='same',
                          activation='relu',
    ...
    convlstm2 = ConvLSTM2D(filters=10,
                          kernel_size=(3,3),
                          strides=(1,1),
                          padding='same',
                          activation='relu',
    ...
    conv3d_output = conv3d_time(input)
    convlstm1_output = convlstm1(conv3d_output)
    # normal_out = normal1(convlstm_output)

    convlstm2_output = convlstm2(convlstm1_output)
    conv3dlayer_output = conv3dlayer(convlstm2_output)
...
test_x = data[2:5]
test_y = label[2:5]
model.fit(data,label,batch_size=2,epochs=20,validation_data=(test_x,test_y))
model.save('conv3d_convlstm_conv3d_0807.h5')

from  keras.models import load_model
model = load_model('conv3d_convlstm_conv3d_0807.h5')
test = data[-1:]
